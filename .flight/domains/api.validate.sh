...
if [[ $# -gt 0 ]]; then
    ...
    mapfile -t FILES < <(find . -type f \( -name "routes*.{js,ts}" -o -name "controller*.{js,ts}" -o -name "*.{js,ts}" -o -name "*Router*.java" -o -name "*Controller*.java" -o -name "views.py" -o -name "urls.py" \) -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -not -path "*/build/*" < /dev/null 2>/dev/null | sort)
fi
