...
describe('loader', () => {
    ...
    describe('loadRulesFile', () => {
        ...
        it('throws with file path on read error', async () => {
            await assert.rejects(loadRulesFile('/nonexistent/path/rules.json'), /Failed to read rules file:.*nonexistent/);
        });
        it('throws with file path on invalid JSON', async () => {
            const filePath = await createTestFile('invalid.json', '{ not valid json }');
            await assert.rejects(loadRulesFile(filePath), /Invalid JSON in rules file:/);
        });
        it('throws with field name on missing domain', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Missing or invalid 'domain'/);
        });
        it('throws with field name on missing version', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Missing or invalid 'version'/);
        });
        it('throws on missing file_patterns', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Missing or invalid 'file_patterns'/);
        });
        it('throws on missing rules array', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Missing or invalid 'rules' array/);
        });
        it('throws on rule missing required field', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Rule 0 missing 'severity'/);
        });
        it('throws on invalid severity value', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /Rule 0 has invalid severity 'INVALID'/);
        });
        ...
        it('throws when AST rule is missing language', async () => {
            ...
            await assert.rejects(loadRulesFile(filePath), /type 'ast' but missing 'language'/);
        });
    });
});
