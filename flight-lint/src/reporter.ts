 ...
 */
export function formatPretty(summary: LintSummary): string {
  const lines: string[] = [];
  ...
  const groupedByFile = new Map<string, LintResult[]>();
 ...
 */
export function formatSarif(summary: LintSummary): string {
 ...
 */
export function groupBySeverity(results: readonly LintResult[]): Map<Severity, LintResult[]> {
  const grouped = new Map<Severity, LintResult[]>();
 ...
 */
export function groupByRule(results: readonly LintResult[]): Map<string, LintResult[]> {
 ...
 */
function countFailures(results: readonly LintResult[]): number {
  return results.filter(
    (lintResult) => lintResult.severity === 'NEVER' || lintResult.severity === 'MUST'
  ).length;
}
 ...
 */
export function getExitCode(results: readonly LintResult[]): number {
