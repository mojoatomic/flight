...
import type { LintResult, LintSummary } from '../src/types.js';
...
describe('reporter', () => {
  ...
  const sampleSummary: LintSummary = {
    domain: 'code-hygiene',
    fileCount: 5,
  ...
  describe('formatSarif', () => {
    ...
    it('maps NEVER/MUST to error level', () => {
      ...
      const neverResult = sarifResults.find((sarifResult: { ruleId: string }) => sarifResult.ruleId === 'N1');
      const mustResult = sarifResults.find((sarifResult: { ruleId: string }) => sarifResult.ruleId === 'M1');
    ...
    it('maps SHOULD to warning level', () => {
      ...
      const shouldResult = sarifResults.find((sarifResult: { ruleId: string }) => sarifResult.ruleId === 'S1');
  ...
  describe('getExitCode', () => {
    ...
    it('returns 0 for SHOULD-only violations', () => {
      const shouldResults: LintResult[] = [
    ...
    it('returns 0 for GUIDANCE-only violations', () => {
      const guidanceResults: LintResult[] = [
